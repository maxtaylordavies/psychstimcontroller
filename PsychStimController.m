...
function PsychStimController_OpeningFcn(hObject, eventdata, handles, varargin)
    ...
    global moviedirpath paramdirpath kNone ktdtSync ktwoPhoton ktdtUDP kcsUDP ...
    ktdsUDP ktdtPT ktdtPTUDP kintan kWFCA;
    % global serialhandle
    % Stryker
...
function StimType_Callback(hObject, eventdata, handles)
    ...
    if ismember(StimType, [1, 6, 8, 9, 16]) % % drifting or counterphase gratings
    ...
    if ismember(StimType, [1, 2, 6, 8, 9, 11, 12, 13, 16]) % % drifting bars, drifting or counterphase gratings, moving spots
...
function RunBtn_Callback(hObject, eventdata, handles) %#ok
    ...
    try
        ...
        global kNone ktdtSync ktwoPhoton ktdtUDP kcsUDP ktdsUDP ktdtPT ...
        ktdtPTUDP kintan kWFCA; %#ok
        % kcsUDP

        %%% load file with rig-specific parameters
        rigSpecific;
        sockrunning = pnet('udpsocket', runningport); % creates a socket for sending/receiving UDP packets
        ...
        % initialise OpenGL - necessary for OpenGL calls (like ClutBlit)
        InitializeMatlabOpenGL;

        % set psychtoolbox debug level to 1
        ...
        % calculate screen size in degrees 
        ...
        % calculate pixel value for grey for the selected screen
        ...
        elseif ismember(stim, [2 7 11 12 13]) % % for bars and spots and moving spots
            ...
            clut = 0;
        end %     blanktexture(1,1) = Screen('MakeTexture', window, blankimage);

        %% stimulus construction lines ~390-890
        switch stim
            case {1, 6, 8, 9} %%%%%%  drifting, counterphase, reverse,curved
                textures = zeros(nCond, 1);
                maskTexture = zeros(nCond, 1);

                for c = 1:nCond
                    if get(handles.StimType, 'Value') == 1 | get(handles.StimType, 'Value') == 16 % % % drift gratings
        ...
        %% gamma correction  Load gamma table if existing - YJS March 2017
        if exist('MyGammaTable.mat', 'file')
            load MyGammaTable;
            gamma_clut = gammaTable * [1 1 1];
        else
            screen_gamma = 1.83
